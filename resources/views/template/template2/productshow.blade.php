...
@if($product->price_visibility)
		<div itemprop="offers">
			...
			<p class="price">
					@if($product->discount)
				<span class="electro-price"><del><span class="amount">{{number_format($product->price)}}</span></del> <ins><span class="amount">{{number_format($product->selling_price())}} {{$product->currency()}} </span></ins></span>
				@else
		...
		<form class="variations_form cart" action="{{route('cart.store')}}" method="post">
            ...
            <div class="hover-area">
                ...
                <a href="https://wa.me/{{$settings->whatsapp}}?text=I'm%20inquiring%20about%20{{urlencode(route('products.show',$product->id))}}" rel="nofollow" title="Inquire this product" > <i class="fa fa-whatsapp" style="font-size:15px;color:green;"></i> Whatsapp</a>
